...
## Abstract  
_Background:_ Breast cancer is the most common cancer worldwide and a leading cause of mortality in women. Cellular heterogeneity within the tumour microenvironment (TME) complicates cell type classification and gene expression analysis, limiting the effectiveness of conventional methods. Foundation models offer a promising approach to overcome these analytical challenges.

_Objectives:_ This study compares three embedding strategies for single-cell RNA sequencing: scGPT (a single-cell foundation model), Mixedbread (a semantic language model), and scMIXE, which integrates both embeddings to improve cell type classification and marker gene discovery.

_Methods:_ Using data from 236,363 cells and 58,892 genes (Xu et al., 2017), embeddings from scGPT and Mixedbread were concatenated to form scMIXE. Clustering was performed using k-nearest neighbour graphs and Louvain modularity optimization. Marker genes were identified by differential expression, and clusters were annotated using the Blueprint/ENCODE reference. Gene set enrichment and ablation analyses were used to compare the performance of all three embeddings.
